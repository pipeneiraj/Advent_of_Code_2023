...
def order_by_type(hands: list[dict[Any, Any]]) -> list[dict[Any, Any]]:
    # 6: Five of a kind
    # 5: Four of a kind
    ...
    for hand in hands:
        ...
        if size == 5:
            ...
            if cards.count(cards[0]) == 2 or cards.count(cards[0]) == 3:
                full.append(hand)
                hand["type"] = 4
            else:
                four.append(hand)
...
def replace_j(cards: str) -> str:
    ...
    if "J" in cards:
        for i, card in enumerate(cards_set):
            if card != "J":
                max_count.append(cards.count(card))
                cards_count.append({"card": card, "count": cards.count(card)})

        for card in cards_count:
            if card["count"] == max(max_count):
                cards_new = cards.replace("J", card["card"])
                break
